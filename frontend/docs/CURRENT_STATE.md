...
A READ-ONLY explanatory layer over the accepted decision trace. **DECISION-001 behavior UNCHANGED** — no threshold lowered, no analyzer weight changed, no confidence math changed, no family-correlation changed, no risk semantics changed, no LockedPrediction semantics changed. No files under `src/domain/{decision,analysis,session,config}` changed.
